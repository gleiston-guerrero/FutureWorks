...
% Evita desbordes de margen por rutas largas en \texttt y URLs
\usepackage[htt]{hyphenat}
\emergencystretch=3em
\tolerance=2000
\hyphenpenalty=1000
\Urlmuskip=0mu plus 1mu\relax
\expandafter\def\expandafter\UrlBreaks\expandafter{\UrlBreaks\do\/\do\-\do\_\do\.}
...
\newtcolorbox{evidbox}[1][]{colback=teal1!7,colframe=teal1,boxrule=0.8pt,arc=2pt,breakable,
...
\begin{tabularx}{\linewidth}{@{}>{\bfseries\color{uteqgreen}}L{0.26\linewidth} X@{}}
    ...
    Marco ético: & Paquete Integral de Anexos y Guías de Elaboración --- Solicitud de Aprobación Ética de los Proyectos Fin de Curso (ISR-401, 2026--2027 PPA). La documentación ética aprobada reposa en \texttt{08\_Etica/} y su vigencia es condición de admisión (gatekeeper G8).\\
...
\begin{notabox}[title=Cláusula de armonización con el Paquete de Anexos y Guías de Aprobación Ética]
    Esta guía y el Paquete Integral de Anexos y Guías de Elaboración forman un solo cuerpo normativo y deben cumplirse simultáneamente.
    Primero, nomenclatura única de entregas: Entrega 1 (1A, semana 4), Entrega 2 (1B, semana 10), Entrega 3 (2A, semana 13) y Entrega 4 (2B/Defensa, semana 17); ambos documentos usan esta equivalencia.
    Segundo, carpeta ética: toda la documentación ética reside en \texttt{08\_Etica/}, carpeta que forma parte del árbol obligatorio de la Sección 9.1; la carpeta \texttt{06\_Experimento/} se reserva para el componente empírico.
    Tercero, zonas de evidencia: la evidencia se organiza en zona pública [P] y zona restringida [R] según la Sección 3; ninguna exigencia de esta guía se cumple publicando datos identificables.
\end{notabox}
...
El paquete ético base del proyecto (documentos A.1 a A.13 del Anexo A más los específicos de su categoría de riesgo) debe estar completo y \textbf{vigente} en \texttt{08\_Etica/}, junto con la \texttt{Adenda\_Segunda\_Ronda.pdf} cargada en la Entrega 3 (2A) con fecha anterior al primer consentimiento nuevo, conforme al Paquete Integral de Anexos y Guías de Aprobación Ética.
Cada participante que se incorpore en esta ronda terminal debe firmar un nuevo consentimiento informado, redactado en cumplimiento de la Ley Orgánica de Protección de Datos Personales del Ecuador \cite{ley_datos_ecuador_2021}, con autorización explícita para el uso de sus datos anonimizados en publicaciones científicas revisadas por pares y para el depósito del conjunto de datos anonimizado en Zenodo con licencia CC BY 4.0.
Ningún consentimiento autoriza la publicación de datos identificables: la firma, la cédula, el rostro y la voz permanecen en la zona restringida [R] definida en la Sección 3; los datos publicados en Zenodo deben estar anonimizados según los principios de minimización y seudonimización de la propia Ley y de las guías de investigación reproducible \cite{molleri2020checklist,wilkinson2016fair}.
...
\begin{evidbox}[title=Zonas de evidencia --- se conservan de la Entrega 3 (2A)]
    Toda la evidencia reside dentro del repositorio, organizada en dos zonas.
    Zona pública [P]: transcripciones anonimizadas, copias enmascaradas de consentimientos (con cédula y firma cubiertas y el código de participante visible), fotografías sin rostros ni coordenadas GPS, respuestas de cuestionario sin columnas identificativas, documentos anonimizados, actas enmascaradas, codificación temática y fichas técnicas de cada archivo multimedia (duración, códec, tamaño y hash SHA-256).
    Zona restringida [R]: material identificable (consentimientos originales, videos y audios sin anonimizar, actas con firma visible y documentos originales), almacenado en \texttt{02\_Evidencias/00\_Restringido/} como contenedor cifrado con AES-256 (7-Zip o VeraCrypt), con la contraseña entregada únicamente al docente por el SGA, nunca dentro del repositorio.
    Verificación: el hash SHA-256 de cada archivo se calcula antes de cifrar y consta en \texttt{checksums.sha256} y en \texttt{fichas\_tecnicas.csv}; un archivo de la ficha que no aparezca en el contenedor, o cuyo hash no coincida, se trata como evidencia ausente o sustituida y dispara el gatekeeper G4.
\end{evidbox}

\begin{longtable}{@{}L{0.28\linewidth} L{0.34\linewidth} L{0.32\linewidth}@{}}
    \hh{Tipo de evidencia} & \hh{Cantidad mínima terminal (objetivo 2B de cierre)} & \hh{Zona, formato y ubicación}\\
    \endfirsthead
    \hh{Tipo de evidencia} & \hh{Cantidad mínima terminal (objetivo 2B de cierre)} & \hh{Zona, formato y ubicación}\\
    \endhead
    Consentimientos firmados & \(\geq 16\) participantes distintos acumulados (24 recomendado), con al menos 3 perfiles bien representados & [R] original cifrado en \texttt{00\_Restringido/} $+$ [P] copia enmascarada en \texttt{Consentimientos/}\\
    Videos reales de entrevistas & \(\geq 16\) archivos, con duración total \(\geq 240\) minutos & [R] contenedor cifrado $+$ ficha técnica pública en \texttt{fichas\_tecnicas.csv}\\
    Audios reales de entrevistas & \(\geq 16\) archivos, redundantes con los videos & [R] contenedor cifrado $+$ ficha técnica pública\\
    Transcripciones anonimizadas & Una por entrevista, en Markdown o JSON estructurado, con seudónimos y código de participante que sustituyan nombres propios & [P] \texttt{02\_Evidencias/Transcripciones/}\\
    Codificación temática cerrada y curva de saturación & Tabla completa $+$ figura de la curva de saturación con inflexión visible (códigos nuevos por entrevista tendiendo a cero) & [P] CSV $+$ PNG/PDF en \texttt{Codificacion\_Tematica/}\\
    Respuestas del cuestionario & \(n \geq 60\) respuestas por perfil dominante, o justificación con cálculo de potencia estadística (Cohen $d=0{,}5$, $\alpha=0{,}05$, $1-\beta=0{,}80$) siguiendo \cite{molleri2020checklist} & [P] CSV/XLSX sin columnas identificativas en \texttt{Cuestionario/Respuestas/}\\
    Documentos originales de la organización cliente & \(\geq 5\) documentos de tipos distintos & [P] anonimizados en \texttt{Documentos\_Organizacion/}; originales en [R]\\
    Sesiones de validación cerradas \emph{(walkthrough)} & \(\geq 6\) sesiones con acta firmada, 3 con usuarios técnicos $+$ 3 con usuarios no técnicos & [P] acta enmascarada $+$ [R] grabación en el contenedor cifrado\\
    Sesión final de miembro-verificación \emph{(member checking)} & 1 sesión con al menos 3 participantes previos del estudio confirmando la interpretación de los resultados \cite{birt2016member} & [P] acta enmascarada $+$ [R] grabación en el contenedor cifrado\\
\end{longtable}
...
\begin{alertbox}[title=Regla estricta de evidencias --- terminal]
    ...
    El docente descifra el contenedor \texttt{02\_Evidencias/00\_Restringido/}, ejecuta \texttt{ffprobe} y \texttt{sha256sum} y contrasta contra \texttt{fichas\_tecnicas.csv}; publicar datos identificables (firma, cédula, rostro, voz o coordenadas GPS) en la zona pública activa el gatekeeper G8, con retiro inmediato del material.
\end{alertbox}
...
\begin{longtable}{@{}p{0.115\linewidth} p{0.185\linewidth} p{0.30\linewidth} p{0.12\linewidth} p{0.20\linewidth}@{}}
...
\begin{evidbox}[title=Árbol de carpetas obligatorio de la Entrega 4 (2B / Defensa)]
    \begingroup
        ...
        \begin{tabbing}
            ...
            \>\>referencias.bib\\
            \>02\_Evidencias/\\
            \>\>00\_Restringido/ [R] cifrado AES-256\\
            \>\>\>evidencias\_restringidas.7z\\
            \>\>\>fichas\_tecnicas.csv\\
            \>\>Consentimientos/ [P] enmascarados\\
            \>\>Transcripciones/\\
            \>\>Fotos\_Entorno/\\
            \>\>Cuestionario/\\
            \>\>\>Fotos\_Aplicacion/\\
            ...
            \>\>\>README\_dataset.md\\
            \>08\_Etica/\\
            \>\>A01\_Protocolo\_Investigacion.pdf ... A13 (Anexo A)\\
            \>\>Categoria\_A/ | \_B/ | \_C/\\
            \>\>Aval\_Institucional.pdf\\
            \>\>Adenda\_Segunda\_Ronda.pdf\\
            \>\>README\_Etica.md\\
            \>09\_Defensa/\\
            \>\>presentacion.pdf\\
            \>\>presentacion.pptx\\
...
\begin{longtable}{@{}L{0.24\linewidth} L{0.70\linewidth}@{}}
    ...
    \texttt{LICENSE} & Licencia MIT o Apache-2.0 para el código del MVP; CC BY 4.0 para el documento y el conjunto de datos anonimizado; \emph{scope} explícito de cada licencia, declarando de forma expresa que el contenido de \texttt{02\_Evidencias/00\_Restringido/} no se licencia, no se redistribuye y no forma parte del depósito abierto.\\
...
\begin{longtable}{@{}p{0.05\linewidth} p{0.87\linewidth}@{}}
    ...
    \hh{\phantom{X}} & \hh{Ítem verificable}\\
    \endhead
    $\square$ & La estructura de carpetas coincide exactamente con la del árbol de la Sección 9.1, incluidas \texttt{02\_Evidencias/00\_Restringido/}, \texttt{08\_Etica/} y \texttt{09\_Defensa/}.\\
    $\square$ & La documentación ética está completa y vigente en \texttt{08\_Etica/}: paquete base A.1--A.13, aval institucional y \texttt{Adenda\_Segunda\_Ronda.pdf} con fecha anterior al primer consentimiento nuevo.\\
    $\square$ & El contenedor \texttt{02\_Evidencias/00\_Restringido/} abre con la contraseña entregada por el SGA y su contenido corresponde a \texttt{fichas\_tecnicas.csv} (\texttt{ffprobe} y \texttt{sha256sum} coinciden).\\
    ...
    $\square$ & Todos los archivos multimedia siguen la nomenclatura \texttt{YYYY-MM-DD\_Tipo\_Codigo\_Tecnica.ext} y usan el código de participante, no el nombre propio.\\
    $\square$ & Cada archivo declarado como audio o video pasa \texttt{ffprobe} sin error y muestra duración mayor a cero.\\
    $\square$ & Cada consentimiento de la zona pública tiene cédula y firma enmascaradas, y su original íntegro está en el contenedor cifrado \texttt{00\_Restringido/}.\\
    $\square$ & Ninguna evidencia publicada conserva datos identificables: fotografías sin rostros ni coordenadas GPS y respuestas sin columnas de nombre, correo, teléfono o IP.\\
...
\begin{landscape}
    ...
    \begin{longtable}{@{}p{0.05\linewidth} p{0.20\linewidth} p{0.35\linewidth} p{0.06\linewidth} p{0.22\linewidth}@{}}
        ...
        C4 & Trabajo de campo terminal, zonas y consentimientos & $\geq 16$ consentimientos (24 recomendado), $\geq 16$ videos ($\geq 240$ min), $\geq 16$ audios, cuestionario $n \geq 60$ por perfil o \emph{power calc}, sesión de \emph{member checking}, zonas [P]/[R] respetadas & 0,50 & Evidencia falsificada o datos identificables publicados $\Rightarrow$ G4/G8\\
...
\begin{itemize}
    ...
    \item \textbf{G4}: Evidencia audiovisual real en la zona restringida cifrada, con hash coincidente en \texttt{fichas\_tecnicas.csv}, \emph{scripts} de análisis reproducibles y referencias verificables. Cualquier evidencia falsificada, ausente del contenedor, con hash no coincidente, cifra no reproducible o referencia inventada $\Rightarrow$ nota máxima 2,50/10 y descalificación del envío del manuscrito.
    \item \textbf{G5}: Cumplimiento de los mínimos empíricos terminales: 16 entrevistas acumuladas (24 recomendado), 60 respuestas por perfil o \emph{power calculation}, curva de saturación con inflexión o \emph{power calc}. Falta $\Rightarrow$ nota máxima 4,00/10.
    ...
    \item \textbf{G8}: Documentación ética vigente en \texttt{08\_Etica/} --- paquete base A.1--A.13, aval institucional y \texttt{Adenda\_Segunda\_Ronda.pdf} con fecha anterior al primer consentimiento nuevo --- y respeto de las zonas [P]/[R] de la Sección 3. Su ausencia $\Rightarrow$ nota máxima 2,50/10; la publicación de datos identificables en la zona pública $\Rightarrow$ 2,50/10 y retiro inmediato del material.
\end{itemize}
...
\begin{longtable}{@{}p{0.08\linewidth} p{0.28\linewidth} p{0.55\linewidth}@{}}
    ...
    14 & Cierre de la recolección de datos & Al menos el 80\,\% de las entrevistas objetivo transcritas (mínimo 16, 24 recomendado); codificación temática avanzada; cuestionario aplicado a $n \geq 45$ por perfil dominante.\\
