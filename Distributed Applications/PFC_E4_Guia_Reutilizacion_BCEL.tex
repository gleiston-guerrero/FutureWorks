...
\fancyhead[L]{\footnotesize\color{uteqBlue}PFC Entrega 4 $\cdot$ Guía de reutilización $\cdot$ Equipo BCEL}
...
\begin{titlepage}
	...
	{\LARGE\bfseries\color{uteqBlue} Equipo BCEL --- SGA Escuela Provincias Unidas\par}
	\vspace{5pt}
	...
	\begin{minipage}{0.93\textwidth}
		...
		\begin{tabular}{@{}p{4.6cm}p{9.8cm}@{}}
			...
			\textbf{Equipo} & BCEL (3--4 integrantes) \\
...
\section{Inventario: lo que BCEL ya tiene}
...
\begin{center}
	...
	\begin{longtable}{@{}p{3.7cm}p{1.5cm}p{5.2cm}p{4.4cm}@{}}
		...
		\addlinespace
		Servidor y clientes de sockets TCP, servicio gRPC, relojes de Lamport & E1 \newline PE-U1 & Se conservan como el canal de sincronización de asistencia desde los dispositivos del aula y como el orden causal de las modificaciones de calificaciones & \reuso \\
		\addlinespace
		...
		\addlinespace
		Clúster CockroachDB de tres nodos y fragmentación por período lectivo y por grado o sección & E3 & \textbf{Sin cambios.} La guía rectora lo dice literalmente: la persistencia distribuida de la E3 se mantiene & \reuso \\
		\addlinespace
...
\section{Lo que la Entrega 4 pide de nuevo al equipo BCEL}
...
\begin{center}
	...
	\begin{tabular}{@{}p{2.5cm}p{12.5cm}@{}}
		\toprule
		\textbf{Web (SPA)} & Portal de docente, secretaría y directivo, con captura de calificaciones y reportes por período \\
		\addlinespace
		\textbf{Móvil} & Cliente móvil para representantes: consulta de calificaciones, asistencia y comunicaciones, con notificaciones \\
		\addlinespace
		\textbf{Patrones GoF} & Repository, Template Method (reportes), Observer (notificaciones), Facade (fachada del portal) y Strategy (algoritmos de promedio) \\
		\bottomrule
	\end{tabular}
\end{center}

\begin{defBox}{Una coincidencia que conviene aprovechar}
	Los cinco patrones asignados a BCEL no son decorativos ni intercambiables: \textbf{Strategy} encapsula los algoritmos de promedio, y es el mismo mecanismo con el que se conmuta el mecanismo de auditoría en la sección 5; \textbf{Template Method} fija el esqueleto común de los reportes por período; \textbf{Observer} dispara las notificaciones cuando se publica una nota; \textbf{Facade} da al portal una entrada única sobre varios microservicios; y \textbf{Repository} aísla el acceso al clúster. Los cinco están en el camino crítico del dominio, así que documentarlos en el ADR-005 es describir lo que el sistema hace, no adornarlo.
\end{defBox}
...
\begin{center}
	...
	\begin{longtable}{@{}p{1.0cm}p{3.5cm}p{1.6cm}p{9.0cm}@{}}
		\toprule
		\textbf{Mód.} & \textbf{Qué exige} & \textbf{Etiqueta} & \textbf{Qué significa concretamente para BCEL} \\
		\midrule
		\endhead
		...
		\addlinespace
		B & Aplicación web (SPA) & \nuevo & Portal de docente, secretaría y directivo con captura de notas y reportes \\
		\addlinespace
		C & Aplicación móvil & \nuevo & Cliente de representante con consulta de notas, asistencia y notificaciones \\
		\addlinespace
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
	...
	\item Extraiga al paquete \texttt{domain} las entidades y objetos de valor del dominio: \texttt{Estudiante}, \texttt{Docente}, \texttt{Matricula}, \texttt{Calificacion}, \texttt{Asistencia}, \texttt{Periodo} y el evento de auditoría. \textbf{Estas clases no llevan anotaciones de persistencia}: las anotaciones se quedan en las clases de \texttt{infrastructure}.
	...
	\item Documente cinco patrones GoF en \texttt{docs/adr/ADR-005-\allowbreak patrones-gof.md} con el formato Nygard. Para BCEL: Repository, Template Method, Observer, Facade y Strategy. Cada uno debe nombrar las clases reales que ocupan cada rol.
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
	...
	\item Cubra al menos cinco rutas. Para BCEL: \texttt{/}, \texttt{/login}, \texttt{/calificaciones}, \texttt{/settings} y \texttt{/about}, con los portales de secretaría y de dirección tras rutas protegidas por credencial y por rol.
	\item La vista principal debe permitir la \textbf{captura de calificaciones y la emisión de reportes por período}, que es lo que la guía rectora asigna a BCEL, y mostrar el historial auditable de cada nota.
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
	...
	\item Listado y detalle del recurso principal, que para BCEL son \textbf{las calificaciones y la asistencia del representado}, con recarga por gesto y modo sin conexión mediante caché local.
	\item Integre al menos dos capacidades del dispositivo. Para BCEL la guía rectora asigna \textbf{notificaciones} (publicación de notas y comunicaciones de la institución); como segunda capacidad se recomienda \textbf{autenticación biométrica} para abrir la aplicación, coherente con el hecho de que muestra datos de menores de edad.
...
\begin{defBox}{Qué evento de negocio debe contar BCEL}
...
\section{Ampliación del Módulo G para el equipo BCEL}
...
En la capa que registra los eventos de calificación, introduzca cuatro comportamientos seleccionables con la variable \texttt{AUDIT}:
...
\begin{center}
	...
	\begin{tabular}{@{}p{1.4cm}p{4.0cm}p{9.6cm}@{}}
		...
		\midrule
		\texttt{m0} & Sin auditoría & Línea base: la nota se escribe y no queda rastro estructurado \\
		\texttt{m1} & Bitácora convencional & Registro de eventos en tabla, sin encadenamiento ni orden causal \\
		\texttt{m2} & Encadenada más Lamport & Cada evento incluye el resumen criptográfico del anterior y una marca lógica de Lamport \\
		\texttt{m3} & \texttt{m2} más relojes vectoriales & Añade reconciliación de ediciones concurrentes y detección de conflictos \\
		\bottomrule
	\end{tabular}
\end{center}

No son cuatro implementaciones ni cuatro ramas: es una estrategia seleccionable, que además es exactamente el patrón \textbf{Strategy} que la guía rectora ya le asigna a BCEL para los algoritmos de promedio. La variable se declara en \texttt{.env.example}.

\subsubsection{Adición 2 --- El inyector de manipulaciones y el verificador de la cadena \emph{[Desarrollador]}}
...
\begin{defBox}{Los cinco tipos de manipulación que hay que inyectar}
	\textbf{T1} Cambiar una nota directamente en la base de datos, sin pasar por la aplicación. \textbf{T2} Eliminar un evento de la bitácora. \textbf{T3} Intercambiar el orden de dos eventos consecutivos. \textbf{T4} Insertar un evento retroactivo con marca anterior a la real. \textbf{T5} Alterar la marca de tiempo de un evento existente.\\[5pt]
	Se inyectan veinte de cada tipo sobre \texttt{m1}, \texttt{m2} y \texttt{m3}. La tabla resultante de mecanismo por tipo es la figura principal de la evaluación.
\end{defBox}
...
\begin{avisoBox}{La detección y la sobrecarga se leen juntas}
...
Los escenarios 2 y 3 se ejecutan bajo los cuatro mecanismos; el escenario 4 solo bajo \texttt{m2} y \texttt{m3}, que son los únicos que reconcilian; el escenario 1 solo bajo la configuración predeterminada. Con las diez repeticiones del protocolo rector son \textbf{110 corridas}, a las que se añade el bloque de detección.

\begin{pisoBox}{Dos comprobaciones sin las cuales el experimento no vale}
	\textbf{Control negativo.} Bajo \texttt{m0} y \texttt{m1}, la manipulación de tipo T1 --- cambiar la nota directamente en la base de datos --- debe pasar inadvertida. Si el sistema la detecta igualmente, es que la inyección no está tocando el dato real y toda la comparación queda invalidada.\\[5pt]
	\textbf{Datos sintéticos sin excepción.} El sistema trata datos de menores de edad. Ningún dato real de estudiante puede aparecer en el repositorio, en los resultados, en el documento ni en las capturas.
\end{pisoBox}
...
\begin{center}
	...
	\begin{longtable}{@{}p{5.4cm}p{1.9cm}p{7.5cm}@{}}
		...
		\texttt{experimentos/\allowbreak inyector\_manipulaciones.py} & \nuevo & Adición 2 de esta guía \\
		...
		\texttt{.env.example} & \cambia & Añadir las variables de web, móvil y \texttt{AUDIT} \\
...
\begin{center}
	...
	\begin{longtable}{@{}p{2.6cm}p{1.5cm}p{5.2cm}p{5.0cm}@{}}
		...
		\addlinespace
		Sábado 29 & 17 & \textbf{Módulo C, día completo}: autenticación, listado y detalle & La app móvil arranca, autentica y consulta calificaciones reales \\
		\addlinespace
		Domingo 30 & 17 & Módulo C: notificaciones y apertura con biometría. Módulo B: rutas y \emph{login} & Las dos capacidades del dispositivo funcionan \\
		\addlinespace
		...
		\addlinespace
		Martes 1 & 18 & Adiciones 1 y 2: \texttt{AUDIT} y el inyector de manipulaciones. Módulo F & \texttt{CORREL} conmuta; el panel muestra las seis vistas \\
		\addlinespace
		Miércoles 2 & 18 & \textbf{Módulo G}: las 100 corridas, lanzadas por lotes desde la mañana & Los datos crudos y los informes del verificador en el repositorio \\
		\addlinespace
...
\begin{center}
	...
	\begin{longtable}{@{}p{0.9cm}p{5.4cm}p{3.0cm}p{5.0cm}@{}}
		...
		\midrule
		\endhead
		1 & La aplicación móvil, por completo & \textbf{Factor 0,5 sobre toda la nota} & Que arranque, autentique contra \texttt{/auth/login}, consulte calificaciones y use dos capacidades del dispositivo \\
		\addlinespace
		2 & La aplicación web, por completo & \textbf{Factor 0,5 sobre toda la nota} & Cinco rutas, \emph{login} con credencial y la captura de calificaciones \\
		\addlinespace
...
\begin{enumerate}[leftmargin=*,itemsep=3pt]
	\item Si alguien con acceso a la base de datos cambia una nota sin pasar por la aplicación, ¿lo detectan? ¿Con cuál de los cuatro mecanismos y en cuánto tiempo?
	\item ¿Cuál es la sobrecarga medida del mecanismo encadenado frente a no tener auditoría, y qué significa para un docente que carga cincuenta notas seguidas?
	\item ¿Cómo garantizan, con relojes lógicos, el orden real de dos modificaciones hechas desde nodos distintos sin reloj común?
	\item ¿Por qué los datos de los experimentos son sintéticos y qué habría hecho falta para poder usar datos reales?
\end{enumerate}
...
\begin{center}
	...
	\begin{tabular}{@{}p{0.7cm}p{14.2cm}@{}}
		\toprule
		\textbf{$\square$} & \textbf{Verificación adicional para BCEL} \\
		\midrule
		$\square$ & \texttt{AUDIT} conmuta entre \texttt{m0}, \texttt{m1}, \texttt{m2} y \texttt{m3} sin tocar código ni cambiar de rama \\
		$\square$ & El inyector registra qué manipulación aplicó, sobre qué evento y cuándo \\
		$\square$ & \textbf{Control negativo validado}: bajo \texttt{m0} y \texttt{m1} la manipulación T1 pasa inadvertida \\
		$\square$ & Los cinco tipos de manipulación están inyectados sobre los tres mecanismos con bitácora \\
		$\square$ & Detección y sobrecarga se reportan juntas, nunca una sola \\
		$\square$ & Las diez repeticiones por escenario están completas y se descartaron la primera y la última \\
		$\square$ & \textbf{No hay ni un solo dato real de estudiante} en ningún artefacto entregado \\
		$\square$ & La captura de calificaciones y los reportes por período funcionan en el portal web \\
		$\square$ & La app del representante consulta notas y recibe notificaciones \\
		$\square$ & Los cuatro contadores de eventos de negocio de BCEL aparecen en el panel \\
		...
		$\square$ & Están los datos crudos de las 110 corridas y del bloque de detección \\
		$\square$ & La comparación entre mecanismos reporta mediana, prueba estadística y tamaño del efecto \\
