...
BLOCKS = [
    ...
    ("Pre-consent tracking", [
        ("Tracking cookie before consent",     43, 40),
...
# ===========================================================================
# Figura E1 — composicion del grupo de referencia por pais, agrupada por region
# ===========================================================================
# Se ordena por region, en el mismo orden que la Tabla 2 del articulo, y dentro
# de cada region por numero de instituciones, para que la figura dialogue con ella.
REGIONES = [
    ("United States",      [("United States", 24)]),
    ("United Kingdom",     [("United Kingdom", 7)]),
    ("Continental Europe", [("Germany", 3), ("Switzerland", 2), ("France", 2),
                            ("Sweden", 1), ("Netherlands", 1), ("Belgium", 1)]),
    ("China",              [("China", 6)]),
    ("Hong Kong SAR",      [("Hong Kong SAR", 3)]),
    ("Rest of Asia",       [("Singapore", 2), ("Japan", 2), ("South Korea", 1)]),
    ("Australia",          [("Australia", 5)]),
    ("Canada",             [("Canada", 3)]),
]
PAISES = [pv for _, ps in REGIONES for pv in ps]
...
# Separadores y etiquetas de region
fila = 0
for nombre, paises in REGIONES:
    n_reg = len(paises)
    total = sum(v for _, v in paises)
    if fila > 0:
        ax.axhline(fila - 0.5, color="#999999", lw=0.7, ls=":", zorder=1)
    ax.text(1.015, fila + (n_reg - 1) / 2.0, "%s (%d)" % (nombre, total),
