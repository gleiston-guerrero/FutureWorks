...
# Taxonomia ampliada (ago-2026): se reclasifica desde los NOMBRES guardados, de
# modo que los datos ya recogidos se reanalizan sin volver a medir.
_PATRONES = [
    r"^_ga", r"^_gid$", r"^__utm", r"^_gcl_", r"^_gac_", r"^receive-cookie-deprecation$",
    r"^YSC$", r"^VISITOR_INFO", r"^VISITOR_PRIVACY", r"^__Secure-YNID$",
    r"^__Secure-ROLLOUT", r"^IDE$", r"^test_cookie$", r"^DSID$",
    r"^_fbp$", r"^_fbc$", r"^fr$", r"^_tt_", r"^_ttp$", r"^ttcsid",
    r"^_clck$", r"^_clsk$", r"^MUID$", r"^MR$", r"^SM$", r"^SRM_B$", r"^ANONCHK$", r"^CLID$",
    r"^_uet", r"^bcookie$", r"^bscookie$", r"^lidc$", r"^li_sugr$", r"^li_gc$",
    r"^UserMatchHistory$", r"^AnalyticsSyncHistory$", r"^TapAd_", r"^sa-user-id",
    r"^_scid", r"^sc_at$", r"^muc_ads$", r"^personalization_id$", r"^_twpid$",
    r"^_hj", r"^nmstat$", r"^_pk_", r"^pys", r"^sbjs_", r"^_pin_",
    r"^AMCV_", r"^s_", r"^utag", r"^mbox", r"^_cs_",
    r"^Hm_lvt_", r"^Hm_lpvt_", r"^HMACCOUNT", r"^BAIDUID$",
    r"^__qca$", r"^ln_or$", r"^_lc2_",
]
_RX = [__import__("re").compile(x) for x in _PATRONES]


def es_rastreo(nombre):
    return any(rx.match(nombre) for rx in _RX)
...
def _rastreo(r):
    ...
    if not c:
        return None
    nombres = c.get("nombres_pre")
    if nombres is None:                      # datos antiguos sin la lista de nombres
        return c.get("rastreo_pre", 0) > 0
    return any(es_rastreo(n) for n in nombres)
