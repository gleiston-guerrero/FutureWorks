...
where node >nul 2>&1
if errorlevel 1 (
	echo   ERROR: no se encuentra Node.js. Ejecute primero INSTALL.bat
	echo.
	pause
	exit /b 1
)
if not exist "audit_multivantage.js" (
	echo   ERROR: falta audit_multivantage.js en esta carpeta.
	echo.
	pause
	exit /b 1
)
if not exist "universities.json" (
	echo   ERROR: falta universities.json en esta carpeta.
	echo.
	pause
	exit /b 1
)
if not exist "node_modules\playwright" (
	echo   ERROR: faltan las dependencias. Ejecute primero INSTALL.bat
	echo.
	pause
	exit /b 1
)
...
for /L %%N in (1,1,20) do (
	if exist "results_%VANTAGE%_r%%N.json" (
		set "YA=!YA! %%N"
		set /a NYA+=1
	)
)
...
set "CUANTAS="
set /p CUANTAS="  Cuantas pasadas NUEVAS quiere run ahora: "
set "CUANTAS=%CUANTAS: =%"
...
for /L %%N in (1,1,20) do (
	if !CUENTA! LSS %CUANTAS% (
		if not exist "results_%VANTAGE%_r%%N.json" (
...
echo.
echo ===========================================================================
echo   Se van a run estas pasadas del punto %VANTAGE%:!PENDIENTES!
...
echo       cd ..\analysis
echo       python verify_multivantage.py
echo.
