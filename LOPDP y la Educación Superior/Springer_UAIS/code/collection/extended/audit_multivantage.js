 ...
 *      node audit_multivantage.js --vantage=EC --run=1 --red=residencial
 *      node audit_multivantage.js --vantage=EU --run=1 --red=centro_datos
 *      node audit_multivantage.js --vantage=EU --run=1 --red=residencial
 *      node audit_multivantage.js --vantage=CH --run=1 --red=centro_datos
 *
 *  Prueba piloto con los primeros 5 sitios:
 *      node audit_multivantage.js --vantage=EC --run=0 --limit=5
 * ==========================================================================*/
...
if (!VANTAGES_VALIDOS.includes(VANTAGE)) {
	...
	console.error('       Ejemplo: node audit_multivantage.js --vantage=CH --run=1 --red=centro_datos\n');
	process.exit(1);
}
if (RUN === '' || isNaN(parseInt(RUN, 10))) {
	console.error('\nERROR: falta o es invalido --run (numero de pasada, entero).');
	console.error('       Ejemplo: node audit_multivantage.js --vantage=EU --run=1\n');
	process.exit(1);
}
...
const ARCHIVO_LISTA = 'universities.json';
const SUFIJO        = `${VANTAGE}_r${RUN}`;
const SALIDA_JSON   = `results_${SUFIJO}.json`;
const SALIDA_CSV    = `results_${SUFIJO}.csv`;
...
(async () => {
	...
	for (let i = 0; i < lista.length; i++) {
		...
		const limpio = results.filter(Boolean);
		guardarJSON(limpio);
		guardarCSV(limpio);
		...
		if (((i + 1) % CHEQUEO_CADA === 0) || esUltimo) {
			...
			if (!bien) {
				...
				console.log('  borre results_' + SUFIJO + '.* y meta_' + SUFIJO + '.json.');
