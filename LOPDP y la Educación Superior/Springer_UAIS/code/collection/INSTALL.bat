@echo off
REM ===========================================================================
REM  INSTALADOR — deja el equipo listo para run la auditoria multipunto
...
if errorlevel 1 (
	...
	echo     INSTALL.bat. Windows necesita reabrir la ventana para encontrar
...
echo       audit_multivantage.js  OK
if not exist "universities.json" (
	echo.
	echo   AVISO: todavia no esta universities.json en esta carpeta.
...
if not exist "universities.json" (
	echo   No se puede hacer la prueba: falta universities.json
	echo.
	pause
	exit /b 0
)
...
echo   Borre ahora los archivos results_EC_r0.* y meta_EC_r0.json:
...
echo.
echo   responda S, cierre PowerShell y vuelva a run INSTALL.bat.
echo.
pause
exit /b 1
