...
const fila = o => filas.push(Object.assign({
	...
	cmp_contenedores: nCMP, cmp_elementos_excluidos: nDescartados, script_version: 'v5'
}, o));
...
const cols = ['id','grupo','sigla','url','criterio','regla_act','regla_nombre','elemento_n',
	...
	'cmp_contenedores','cmp_elementos_excluidos','script_version'];
const csv = '\ufeff' + cols.join(',') + '\n' +
...
/* Acumulacion de resumenes. El almacenamiento del navegador es por dominio, de
   modo que no puede compartirse entre los quince sitios; se usa por tanto una
   variable de la propia pestana. Mientras el evaluador no cierre la pestana de
   herramientas, cada ejecucion anade su linea y el portapapeles recibe TODAS
   las acumuladas hasta el momento, no solo la ultima. Asi basta con pegar una
   vez al final. */
window.__actResumenes = window.__actResumenes || [];
window.__actResumenes = window.__actResumenes.filter(x => !x.startsWith(`ACT|v5 sitio=${SITIO.sigla} `));
window.__actResumenes.push(resumen);

const todos = window.__actResumenes.join('\n');
try {
	copy(todos);
	console.log(`(Copiadas al portapapeles ${window.__actResumenes.length} linea(s) de resumen acumuladas en esta pestana.)`);
	console.log('Peguelas en un documento de texto. Si cierra el navegador se pierden, pero cada CSV lleva los mismos datos dentro.');
} catch (e) {
	console.log('No se pudo copiar automaticamente. Seleccione y copie la linea de arriba.');
}
console.table(window.__actResumenes.map(x => {
	const o = {};
	x.split(' ').forEach(p => { const i = p.indexOf('='); if (i > 0) o[p.slice(0, i)] = p.slice(i + 1); });
	return { sitio: o.sitio, filas: o.filas, modal: o.modal, revisar: o.revisar };
}));
